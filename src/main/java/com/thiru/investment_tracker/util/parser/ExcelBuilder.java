...
public class ExcelBuilder {
    ...
    public static ByteArrayInputStream downloadPortfolioStocks(List<AssetResponse> userStocks) {

        ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
        try (XSSFWorkbook workbook = new XSSFWorkbook()) {
            ExcelParser.initialiseExcel(workbook, ExcelHeaders.getPortfolioHeaders(), ExcelParser.ASSETS);
    ...
    public ByteArrayInputStream downloadTransactions(List<TransactionResponse> userTransactions) {
        ...
        ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
        try (XSSFWorkbook workbook = new XSSFWorkbook()) {
            ExcelParser.initialiseExcel(workbook, ExcelHeaders.getTransactionHeaders(), ExcelParser.TRANSACTIONS);
            Sheet sheet = workbook.getSheetAt(0);
