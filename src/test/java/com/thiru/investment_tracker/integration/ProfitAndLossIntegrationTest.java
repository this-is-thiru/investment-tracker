...
import com.thiru.investment_tracker.service.ProfitAndLossService;
...
public class ProfitAndLossIntegrationTest extends AbstractIntegrationTest {
    ...
    void updateProfitAndLoss_whenSelfShortTermGain_shouldCreatePnl() {
        ...
        assertEquals("2023-2024", pnl.getFinancialYear());
    }
    ...
    void updateProfitAndLoss_whenNewFy_shouldAutoCreateEntity() {
        ...
        assertEquals("2024-2025", pnl.getFinancialYear());
        assertNotNull(pnl.getRealisedProfits());
    }
    ...
    void updateProfitAndLoss_whenWriteFailure_shouldRollback() {
        ...
        // Attempt sell first to create asset state
        try {
        } catch (Exception e) {
            // Expected if stock code doesn't exist
        }

        // Create context with invalid data that should trigger rollback
        ProfitAndLossContext context = ProfitAndLossContext.from();
    ...
    void updateProfitAndLoss_whenExactOneYearBoundary_shouldTreatAsLongTerm() {
        ...
        ProfitAndLossEntity pnl = findPnlByEmail(email);
        assertNotNull(pnl);
        // At exactly 1 year, sellDate is NOT before threshold (purchaseDate + 1 year)
    ...
    @Test
    void updateProfitAndLoss_whenZeroQuantity_shouldRecordZeroGain() {
        // GIVEN
        ...
        AssetRequest sellRequest = createSellRequest(email, stockCode, sellDate, 0.0, 15.0, AccountType.SELF);

        ProfitAndLossContext context = buildContext(email, stockCode, buyDate, sellDate, 0.0, 10.0, 15.0, AccountType.SELF);
        ...
        assertNotNull(pnl);
        assertNotNull(pnl.getRealisedProfits());
        // Zero quantity means zero profit
        assertEquals(0.0, pnl.getRealisedProfits().getShortTermCapitalGains().getProfit(), 0.01);
    }
